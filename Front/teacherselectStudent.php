<?php
session_start();
if( $_SESSION['role'] != "teacher" || isset($_POST['logOut']) ){
    session_destroy();
    header("Location: https://afsaccess4.njit.edu/~lg278/index.php");
    
}

?>

<!DOCTYPE html>
<head>
	<meta charset="utf-8">
 <link rel="canonical" href="https://getbootstrap.com/docs/5.0/examples/sign-in/">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    <link href="style.css" rel="stylesheet">
</head>
<body class="text-center">
<main class="content">

<form method="post">
<button class="btn btn-outline-primary" name="logOut">logout</button>
</form>

<br>
<a href="https://afsaccess4.njit.edu/~lg278/teacherlanding.php">
<button class="btn btn-outline-primary">back to landing page</button>
</a>

<h1>Select a student</h1>


<?php
    $URL= 'https://afsaccess4.njit.edu/~tz6/back/selectStudent.php';
    $ch = curl_init();
    $post = $_POST["exam"];
		curl_setopt($ch, CURLOPT_URL, $URL);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_POSTFIELDS, $post);
		$result = curl_exec($ch);
		curl_close($ch);
    $eArray = json_decode($result, true);
    
   

  ?>

<form type="examSelection.php" method="post" id="selectStudent" action="https://afsaccess4.njit.edu/~lg278/teacherviewExam.php" >

</form>

<script>
      var data = <?php echo json_encode($eArray, JSON_HEX_TAG); ?>;
      var e_id = <?php echo json_encode($post, JSON_HEX_TAG); ?>;
      
      let exam = document.getElementById("selectStudent");
      
      data.forEach((item)=>{
      
      
      
      let eName = document.createElement("label");
      eName.innerHTML = "student ID: " + item[0] + " " + "Last Name: " + item[1] + " " + "First Name: " + item[2];
      let select = document.createElement("input");
      select.setAttribute("type","radio");
      select.setAttribute("name","student");
      select.setAttribute("value",item[0]);
      let br1 = document.createElement("br");
      exam.append(eName);
      exam.append(br1);
      exam.append(select);
      let br = document.createElement("br");
      exam.append(br);
      
      });
      
      let id = document.createElement("input");
      id.setAttribute("name","exam");
      id.setAttribute("type","hidden");
      id.setAttribute("value",e_id);
      exam.append(id);
      
      let b = document.createElement("button");
      b.innerHTML = "Submit Selection";
      b.setAttribute("class","btn btn-outline-primary");
      b.setAttribute("type","submit");
      b.setAttribute("name","submit");
      exam.append(b);
      
         
</script>
<?php

?>

</body>
</html>